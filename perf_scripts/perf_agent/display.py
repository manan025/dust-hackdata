...
def show_llm_optimization_response(response_text: str, iteration: int) -> None:
    from .llm import _C_BLOCK_RE

    # Strip the ```c...``` block — show only the surrounding explanation
    display_text = _C_BLOCK_RE.sub("*(source code block omitted)*", response_text).strip()
    CONSOLE.print(
        Panel(
